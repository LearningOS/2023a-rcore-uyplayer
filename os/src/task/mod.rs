...
impl TaskManager {
    ...
    fn syscall_count_update(&self,syscall_id:usize){
        let  mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].syscall_count[syscall_id] += 1;
    }
    /// set current task info
    fn set_current_task_info(&self) ->TaskInfo{
        // get TaskManager
        let  inner = self.inner.exclusive_access();
        // get current task
        let current = inner.current_task;
        // get whole running time
...
pub fn update_syscall_count(syscall_id: usize) {
    TASK_MANAGER.syscall_count_update(syscall_id)
}

/// set task info
pub fn set_current_task_info() -> TaskInfo {
    TASK_MANAGER.set_current_task_info()
}
